"""
HazeToolPlat Demo MCP Server (HTTP)
实现 MCP JSON-RPC over HTTP，供 Haze 平台 HTTP MCP 托管部署测试。
...
测试接口：
    ...
    curl -X POST http://localhost:8000/mcp \\
         -H "Content-Type: application/json" \\
         -d '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"list_employees","arguments":{}},"id":2}'
"""
...
TOOLS: list[dict] = [
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "department": {"type": "string", "description": "部门名称，留空返回全部"},
    ...
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "employee_id": {"type": "string", "description": "员工工号，格式 EMP-xxx"},
            },
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "days": {"type": "integer", "description": "查询最近几天，默认 7"},
            },
            "required": ["employee_id"],
        },
    },
    {
        ...
        "description": "获取 Demo MCP Server 运行信息",
        "inputSchema": {"type": "object", "properties": {}, "required": []},
    },
]
...
def tool_get_system_info(_args: dict) -> dict:
    ...
    return {
        "server": "HazeToolPlat Demo MCP Server (HTTP)",
        "version": "1.0.0",
...
def handle(body: dict) -> dict | None:
    ...
    if method == "initialize":
        return _ok(req_id, {
            ...
            "serverInfo": {"name": "demo-http-mcp-server", "version": "1.0.0"},
...
@app.get("/health")
def health():
    """健康检查，Gateway 部署后会轮询此接口。"""
    return jsonify({"status": "ok", "uptime_seconds": int((datetime.now() - _START_TIME).total_seconds())}), 200


@app.post("/mcp")
def mcp():
    """MCP JSON-RPC 主入口，接收并分发所有 MCP 请求。"""
